...
export async function PUT(request: Request) {
  try {
    ...
    const { id, name, stock_awal, keluar_manual, keluar_pos, days_to_order } = body as {
      id: number;
      name: string;
      stock_awal: number;
    ...
    const days = days_to_order !== undefined ? days_to_order : (currentProduct.data?.days_to_order || 0);
